...
from labmon.sensors import mock_temperature
from labmon.sensors.mock_temperature import TemperatureWalk, main, run
...
def test_run_writes_points_and_shuts_down_cleanly_on_signal(
    ...
    monkeypatch.setattr(mock_temperature, "get_client", lambda: fake_client)
...
def test_main_parses_defaults_and_calls_run(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(mock_temperature, "run", fake_run)
    monkeypatch.setattr(sys, "argv", ["mock-temperature-sensor"])

    main()

    assert calls == [{"sensor_id": "mock-temp-1", "interval": 5.0, "setpoint": 21.0}]
...
def test_main_parses_custom_arguments(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert calls == [{"sensor_id": "fridge-2", "interval": 1.0, "setpoint": 4.0}]
